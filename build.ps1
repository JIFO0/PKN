...
Get-ChildItem -Path $DestDir -Recurse -File | ForEach-Object {
    $filePath = $_.FullName
    $entryName = $filePath.Substring($DestDir.Length + 1).Replace('\', '/')
    $entryName = "pkn-backend/" + $entryName.Substring($entryName.IndexOf('/') + 1)
    # Naprawia: pkn-backend\assets\css\admin.css → pkn-backend/assets/css/admin.css
